...
- **新订阅商接入必守(所有第三方订阅一律适用)**:
  1. **模型与思考档位必须动态获取,不得写死**——不同订阅档位可用的模型与思考深度不同,写死会导致用户升级订阅后部分模型/档位不可用。选择器只展示账户目录声明的模型与档位;静态列表仅作离线兜底,绝不替代在线结果;
  2. **多思考档位按从低到高排序,首项为 Off**(仿照 deepseek 模型的设计);默认档优先取账户目录声明的默认值,其次才用配置;
  3. **模型声明多模态(图片输入)时必须实测多模态能力**——以真实账户或等效往返测试覆盖用户图片与工具结果图片两条路径(如 openai 与 xai 接入时对图片输入、工具结果图片的验证),发现问题必须修复,验证通过前不得视为接入完成。
