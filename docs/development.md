...
| 9 | 推送被仓库规则拦截(repository rule violations) | 代码内嵌了 OAuth client secret(`GOCSPX-` 触发密钥保护) | `GOCSPX-` 字面量一律不进仓库:公开 client secret 在运行时从官方源读取或经环境变量 `*_OAUTH_CLIENT_SECRET` 提供;公开 client id 可以入库 |
| 10 | API-key 类服务与外壳内置目录重复 | harness 的「模型」页「Add provider」已原生内置多种 API-key 路由(同端点、同凭据方式) | 重复的服务不接入;产品方向定为「仅 OAuth 订阅」后,插件侧的密钥类实现已全部移除,密钥类服务继续由内置目录提供(见 `AGENTS.md` 接入规范第 4 条) |
...
| 13 | 第三方 OAuth 客户端不能声明未注册作用域(`403 restricted_client: Unregistered scope(s)`)且其令牌被目标 API 拒绝(`ACCESS_TOKEN_SCOPE_INSUFFICIENT`) | 服务商官方客户端只注册了自身用途的作用域;为它追加其它作用域会被授权页拒绝,令牌也没有目标 API 的访问权 | 接入前先核实「官方客户端的作用域是否覆盖目标 API」;不覆盖时改用自建客户端(相应 API 在项目内启用,并把账号加为测试用户),经环境变量覆盖客户端身份(`*_OAUTH_CLIENT_ID`/`*_OAUTH_CLIENT_SECRET`) |
